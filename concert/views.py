...
def signup(request):
    if request.method == "POST":
        ...
        try:
            ...
            if user:
                return render(request, "signup.html", {"form": SignUpForm, "error": "User already exists"})
            else:
...
def songs(request):
    songs = req.get("http://songs-sn-labs-ahamson.labs-prod-openshift-san-a45631dc5778dc6371c67d206ba9ae5c-0000.us-east.containers.appdomain.cloud/songs").json()
    return render(request, "songs.html", {"songs": songs["songs"]})

def photos(request):
    photos = req.get("https://pictures.1mii1idl5s1s.us-south.codeengine.appdomain.cloud/picture").json()
